...
bool get_molecule(const string& input_name, unsigned long long quantity, map<string, unsigned long long>& bank) {
    ...
    for (const auto& pair : required_atoms) {// check if the required atoms are available in sufficient quantity
        ...
        unsigned long long amount = pair.second;// get the amount of the atom required for the molecule
    ...
    for (const auto& pair : required_atoms) {// deduct the required atoms from the bank
        ...
        bank[atom] -= static_cast<unsigned long long>(amount) * quantity;// using static_cast to convert the int to unsigned long long
    }
 ...
 * @param bank: A map representing the initial inventory.
 * @return: bool indicating success or failure. 
 */
bool read_save_file(const string& path, map<string, unsigned long long>& bank) {
    ifstream infile(path);// open the file for reading
    if (!infile) return false; // if the file cannot be opened, return false

    string line;
    unsigned long long num;
    while (infile >> line >> num) { // read each line from the file
        bank[line] = num; // update the bank with the values from the file
    }
    return true;
}
...
/*
 * Function to write the current state of the bank to a file.
 * It overwrites the existing file or creates a new one if it does not exist.
 * @param path: The path to the file where the bank will be saved.
 * @param bank: A map representing the current state of the bank.
 * @return: bool indicating success or failure.
 */
bool write_save_file(const string& path, const map<string, unsigned long long>& bank) {
    ...
    FILE* file = fdopen(fd, "w");//file object to write to the file
    if (!file) {
        cerr << "Error opening file stream: " << path << endl;
        close(fd);
        return false; // if the file stream cannot be opened, return false
    }
    for(const auto& pair : bank) { // write each atom and its quantity to the file
        fprintf(file, "%s %llu\n", pair.first.c_str(), pair.second);
    }
    fflush(file); // go over the file stream to ensure all data is written
    flock(fd, LOCK_UN); // unlock the file
    fclose(file); // close the file stream and the file descriptor
    return true; // return true if the file was written successfully
}
...
string parse_command(const string& com, map<string,unsigned long long>& bank){
    ...
    if (save_file) {
        read_save_file(save_file, bank);
    }

    string rest;// get the rest of the command after DELIVER
    getline(iss, rest);// create a string stream to parse the rest of the command
    istringstream rest_iss(rest);// create a string stream to parse the rest of the command
    vector<string> tokens;// to hold the tokens of the command
    string token;
    ...
    string quantity_str = tokens.back();//insert the last token as the quantity
    tokens.pop_back();// remove the last token from the tokens vector
    ...
    if (success) {//print the success message if the molecule can be created
        ...
        if(save_file){//check if the save file is provided
            write_save_file(save_file, bank);
        }
...
int main(int argc, char *argv[]) {
    ...
    if (save_file && access(save_file, F_OK) == 0) {// Check if the save file exists
        cout << "Updating inventory from file: " << save_file << endl;//update information from the file
        if (!read_save_file(save_file, bank)) {// if the file cannot be read, return an error
    ...
    if (has_uds_stream) {
        ...
        if (listen(uds_fd, BACKLOG) < 0) {// listen for incoming connections on the UDS stream socket
        ...
        for (int i=0;i<num_clients;i++) {
            if (FD_ISSET(client_socket[i], &read_fds)) {
                ...
                if(valread <= 0) {
                    ...
                    // this is to ensure that the bank is updated with the latest values from the save file
                    if (save_file) {
                        read_save_file(save_file, bank);
                    }
                    ...
                    if(a=="ADD"&&bank.count(type)){// check if the command is ADD and the atom type is valid
                        if(bank[type] + quantity <= MAX_CAPACITY) {// check if the quantity is within the maximum capacity
                            ...
                            if(save_file){// check if the save file is provided and update it
                                write_save_file(save_file, bank);// write the bank to the save file
                            }
