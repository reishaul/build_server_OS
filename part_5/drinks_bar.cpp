...
int main(int argc, char *argv[]) {
    ...
    // Define variables for UDS paths- NEW
    string uds_stream_path, uds_dgram_path;
    // Flags to check if UDS paths are provided- NEW
    bool has_tcp = false, has_udp = false, has_uds_stream = false, has_uds_dgram = false;

    // Parse command line arguments
    while((input=getopt_long(argc, argv, "T:U:t:o:h:c:s:d:",long_options, nullptr))!=-1) {
    ...
    while (true) {
        ...
        // NEW: before the select- check if provided and add UDS sockets to the set if they exist
        if (has_uds_stream) {
